...
## Release
Use `labelvier release` to start and finish releases
```
Available functions:
...
## Staging
When asking 'deploy to staging' or 'zet op staging' or 'test op staging', use `labelvier deploy staging`. Don't run `npm run deploy-staging` directly. If you have merge conflicts during staging deploy, fix and push the `staging` branch. After that run `labelvier deploy staging` again.
