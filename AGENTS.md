...
Label Vier CLI — pure-bash command tool (`labelvier`, shorthand `l4`; the old name `wp-takeoff` still works but is deprecated) for the Label Vier WordPress Starter Kit. No build step, no dependencies; bash sourced at runtime.

## Layout

- `labelvier` — entry point (real file). `l4` and `wp-takeoff` are symlinks to it — one source of truth, dispatched on by `$(basename "$0")`. Sources every `inc/*.sh`, then `main "$@"` runs `$1` as a function.
...
- `templates/example.sh.tpl` — scaffold template (`labelvier core generate`).
...
- A typo'd subcommand (non-empty, not flag-like, no matching function) prints `✗ Unknown command: <name>` before falling back to the docs, and returns/exits non-zero — distinct from the bare-call case (no error, docs only). `_dispatch`, the top-level `labelvier` entry script, and `ai.sh`'s nested `claude` all do this; mirror it in any other hand-rolled dispatcher.
- Color vars `$__red $__blue $__green $__bold $__reset` are set in `labelvier` and inherited — use them, don't redefine.

## Adding a command

1. Read `inc/hello.sh` (minimal) and `inc/cron.sh` / `inc/release.sh` (real) first; mirror their structure.
2. Scaffold with `labelvier core generate <command-name> <first-command-name>` — both args are optional and, if omitted, are asked for interactively; passing them makes it a non-interactive one-liner (the form an agent should use). Or copy the pattern from `templates/example.sh.tpl` by hand.
...
- `./labelvier <cmd>` — prints docs (confirms dispatch + parsing).
- `./labelvier <cmd> <sub>` — runs subcommand.
- Never run destructive, remote (ssh/scp), or quota-spending subcommands just to "test". Confirm dispatch + syntax, or ask.

## Release

Bump `version` in `package.json` only when asked. The tool self-updates via `git pull` (`core update`); remote is GitHub (`labelvier/cli`).
